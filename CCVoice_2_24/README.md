...
```
CCVoice_2_24/
  ├── CCVoice_2_24.ino    メインスケッチ（必須）
  ├── ccvoice_log.h       イベントログ関連（必須）
  └── ccvoice_config.h    設定マイグレーション関連（必須）
```

> **3ファイルを同一フォルダに置いて** Arduino IDE で `CCVoice_2_24.ino` を開いてください。
...
```
[START] OpenCCVoice v2.24 (Event Log, Dual EEPROM, RTC-Aligned Periodic, Ver.5 pinmap)
[RTC] DS3231 found.
