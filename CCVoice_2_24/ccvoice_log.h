...
// ログヘッダ読み込み
inline bool logReadHeader(LogHeader &h) {
  extEepromRead(LOG_HDR_ADDR, (uint8_t*)&h, sizeof(h));
  return (h.magic == LOG_MAGIC);
}

// ログヘッダ書き込み
inline void logWriteHeader(const LogHeader &h) {
...
// エントリ1件書き込み
inline void logWrite(uint8_t evt, uint32_t data) {
...
// 時刻文字列出力ヘルパー
inline void printLogTime(uint32_t ts) {
