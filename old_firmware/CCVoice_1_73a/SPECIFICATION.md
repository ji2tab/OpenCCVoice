...
(3) AUTO（m2）
  ...
  - AUTO固定時は D13 LED を高速点滅（約3秒）で通知。
...
(5) EEPROM 永続化
  - m / n / b / i / p / r / L / G / a / w / s / t / d / g
    すべての設定変更で即保存。
  - 起動時は EEPROM からロード。magic 不一致時は applyDefaults()。

(6) LED可視化 / D2ミラー出力
  - 使用側LED（D6系=D4, A0系=D12）は実際のBUSYを表示。
  - 未使用側LEDはゆっくり点滅（待機表示）。
  - 抑止LED（D13）は抑止・バースト・AUTO固定通知を表現。
...
(7) フェイルセーフ（PTT張り付き防止）
  ...
  - d0 で無効化可能（無効時は PLAYING 内で判定スキップ）。
...
- PTT 最低ONガードを搭載し、誤送信防止。
- 可搬性：Arduino IDE 1.8.x / 2.x。
- 保守性：printSummary() による状態可視化、ログレベル制御。
- 安全性：フェイルセーフ（既定20秒、d####で可変、d0で無効）。
...
- D13 : 抑止/AUTO LED（抑止・短発窓・AUTO固定の可視化）
...
- q                    … 現在値の要約表示（DFP_TIMEOUT / BUSY極性含む）
...
- D6固定（m0）  
  600ms BUSY → ID送出  
  4000ms BUSY → 長話抑止  
- AUTO：m2 → w1分 → 観測満了 → [AUTO-FIXED] & D13高速点滅  
- 送信後抑止：t1 / r3000 → 自局送出後3s無視  
- フェイルセーフ：d20000 → BUSY戻らず20秒経過で強制OFF  
  d0 → フェイルセーフ無効  
- TM BUSY極性  
...
10. リスク
========================================================================
- DFPlayer BUSY の個体差（LOW/HIGH の応答速度差）
- 外部回路のプルアップ不足による D6/A0 不安定
- A0/D6 の同時配線は禁止（AUTO は判定のみであり、配線は常に片側）
- 無線機 BUSY の極性仕様を誤ると短発/長話判定が乱れるため g0/g1 を適切に設定
