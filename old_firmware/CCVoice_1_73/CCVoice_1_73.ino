 ...
 * - フェイルセーフ（DFPlayer応答なし時の強制PTT OFF）を可変化
 *    - 既定: 20000ms（20秒）
 *    - コマンド d#### で変更、d0 で無効化（EEPROM永続化）
...
void saveSettings() {
  ...
  config.dfpTimeoutMs = DFP_TIMEOUT_MS; // ★保存
  EEPROM.put(0, config);
...
void printSummary() {
  ...
  Serial.print(F(" DFP_TIMEOUT(ms)=")); Serial.println(DFP_TIMEOUT_MS);
}
...
void handleSerialCmd() {
  while (Serial.available()) {
    ...
    switch (c) {
      ...
      case 'd': if (val>=0)   DFP_TIMEOUT_MS = (unsigned long)val; else chg=false; break; // ★追加
...
void setup() {
  ...
  Serial.println(F("[START] OpenCCVoice v1.73a Unified/Safe (+DFP Timeout Cmd)"));
  printSummary();
  printHelp();
}
