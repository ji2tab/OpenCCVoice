 ...
 *  - AUTO窓       : w##（分）
 *  - 要約/停止    : q / x→停止, r→再開
 *  - プリセット   : H（s0/t0/b3900）※装置名を特定しない「汎用プリセット」
...
/* ============================================================
   ■ ピン定義（v1.69g配線準拠）
   ============================================================ */
/* TM BUSYの極性（回路の実態が HIGH=受信 の場合は true） */
const bool TMBUSY_ACTIVE_HIGH = true;

/* DFPlayer BUSY -> D2 ミラー出力（反転：再生中HIGH／停止中LOW） */
...
const uint8_t PIN_DFP_BSY  = 7;     // D7 : DFPlayer BUSY（多くの個体で LOW=再生中）
const uint8_t PIN_DFP_OUT  = 2;     // D2 : D7 BUSYミラー出力（反転仕様）
const uint8_t PIN_SUP_LED  = 13;    // D13: 抑止/AUTO通知LED
const uint8_t PIN_A0_LED   = 12;    // D12: A0状態表示LED
const uint8_t A0_PIN       = A0;    // A0 : アナログ入力

/* --- DFPlayerのシリアル：Arduino視点で明確化（配線は従来どおり） --- */
// DFPlayer TX(モジュール送信) -> Arduino D10（ArduinoのRX）
// DFPlayer RX(モジュール受信) <- Arduino D11（ArduinoのTX）
const uint8_t ARD_RX_FROM_DFP = 10; // Arduino RX（DFP TXが入る）
const uint8_t ARD_TX_TO_DFP   = 11; // Arduino TX（DFP RXへ送る）
...
void applyDefaults() {
  ...
  BUSY_MIN_MS = 500;
  BUSY_MAX_MS = 3900;   // 汎用プリセット相当の既定（長文閾値）
  IDLE_MIN_MS = 200;
...
void printHelp() {
  ...
  Serial.println(F("p##=period(min), L###/G###/a####=A0 lo/hi/hold(ms), w##=AUTO(min)"));
...
void setup() {
  ...
  Serial.println(F("[START] OpenCCVoice v1.73 Unified/Safe (HW:v1.69g配線維持 / +Failsafe)"));
  printSummary();
  printHelp();
}
