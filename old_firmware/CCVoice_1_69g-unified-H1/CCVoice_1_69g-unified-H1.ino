 ...
 * Version : 1.69g-unified-H1-b3900 (H1カーチャンク即復旧プリセット付き, hotfix)
 ...
 *   - CHUNK上限       : 3900ms（LONG_TALK_MS も 3900ms に自動ペア）
 *   - D4上限超過点滅 : OFF（純受信LED）
 *   - A0運用時        : HOLDは 800ms（必要なら600ms版も別ビルド可）
 *
 * ■ ワンキー（シリアル）
 *   - 'H' : H1クイックプリセット適用（s0 / t0 / l0 / b3900）
...
unsigned long BUSY_MAX_MS                = 3900;  // ★H1既定：3.9s（H1現場 2.3〜3.9s をフル許容）
...
/* ======= 文脈抑止 ======= */
unsigned long LONG_TALK_MS            = 3900;     // ★H1既定：BUSY_MAX_MSにペア
...
void printHelp() {
  ...
  Serial.println(F(" b####   : set BUSY_MAX_MS to #### (e.g., b3000/b3900); LONG_TALK_MS pairs"));
  Serial.println(F(" q       : print current configuration summary"));
  Serial.println(F(" H       : apply H1 quick preset (s0 / t0 / l0 / b3900)"));
}
...
void applyH1Preset() {
  ...
  BUSY_MAX_MS = 3900; LONG_TALK_MS = BUSY_MAX_MS;
  Serial.println(F("[H1] preset applied: s0 / t0 / l0 / b3900"));
  printSummary();
}
...
void setup() {
  ...
  Serial.println(F("[BOOT] 1.69g-unified-H1-b3900 (H1プリセット既定)"));
  printHelp();
  printSummary();
}
