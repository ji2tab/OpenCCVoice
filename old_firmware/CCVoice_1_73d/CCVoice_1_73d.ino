 ...
 *
 * 【v1.73e 修正点（本流）】
 * - ★周期IDの「追い付き（catch-up）」連続発火を根治：nextPeriodicAt を while で未来へ追い付かせる
 * - ★送信直後の誤発火（A0残留BUSY/スケルチテール等）を根治：postTxIgnore ガード追加
 * - ★A0 BUSY観測は抑止中でも継続（観測停止による誤判定を防止）
 *
 ...
 * - v1.73e は挙動改善のみ（レイアウト変更なし）→ ver=4 のまま
 */
struct MyConfig {
  uint32_t magic;
  ...
  uint8_t  ver;              // ★EEPROMレイアウト版 (v1.73d/e=4)
} config;

const uint32_t CONFIG_MAGIC   = 0xDEADBEEF;
const uint8_t  CONFIG_VERSION = 4;  // レイアウト番号（変更なし）
...
// ★v1.73e: 送信後、BUSYが十分落ち着くまでイベント発火を禁止
...
static inline bool isSuppressedNow(unsigned long now) {
  if (postTxIgnore) return true;     // ★v1.73e: 送信後落ち着くまで抑止扱い
...
void setup() {
  ...
  Serial.println(F("[START] OpenCCVoice v1.73e Unified/Safe (+Catch-up fix +PostTx guard)"));
...
void loop() {
  ...
  // A0判定（★抑止中でも“BUSY状態の観測”は継続する）
  // ただしAUTO用のイベントカウントは抑止中は増やさない
  if (!pAct) {
  ...
  // 周期ID スケジューラ（★追い付き処理で nextPeriodicAt を必ず未来へ）
  if (PERIOD_MS > 0) {
    bool crossed = false;
  ...
  switch (state) {
    ...
    case PTT_OFF_WAIT:
      if (now - stateTimer >= PTT_POST_MS && now >= pttMinOn) {
        ...
        // ★v1.73e: 送信直後の誤発火対策（A0残留BUSY/テール等）
        postTxIgnore = true;
        postTxIdleStart = 0;

        // ★直後の再発火を防ぐ保険（不応期の起点更新）
        lastTriggerAt = now;
