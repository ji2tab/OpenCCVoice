...
- IDLE → PTT_ON_WAIT（PRE）
       → PLAYING（DFPlayer BUSY: LOW→HIGH）
       → PTT_OFF_WAIT（POST）
       → IDLE
...
========================================================================
- D6 デバウンス 5ms（固定）
- A0/D6 の同時配線は禁止（AUTOは論理判定であり物理接続は片側）
- EEPROM 書込みは「値変更時のみ」
- EEPROM耐久性に配慮（無関係な再書き込みを避ける）
- SRAM：ATmega328P の 2KB 内で動作（最適化済み）
...
- 周期ID静寂時間（k####）を短くしすぎると「割り込み感」が戻る

========================================================================
11. 変更履歴
========================================================================
- 詳細は CHANGELOG.md（v1.73d 対応版）を参照
