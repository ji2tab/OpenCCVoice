...
## v2.23 — コンパイルエラー修正版 (2026-03-12)

### Fixed
- **v2.22 のコンパイルエラーを修正**
  - `enum BusySrc` / `enum LogLvl` の定義が `struct LogHeader` 等より
    後ろに配置されていたため、コンパイル時に未宣言エラーが発生していた問題を修正。
  - 定義順を `enum` → `struct` → `変数` → `関数` に整理。

### Notes
- 機能的な変更はありません。v2.22 と動作は同一です。
- v2.22 は廃止。GitHub からも削除します。
