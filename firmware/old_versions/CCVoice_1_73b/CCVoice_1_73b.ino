 ...
 * - EEPROMに version stamp（config.ver）を導入し、自動初期化／移行に対応
...
/* =============================== EEPROM =============================== */
/* 互換性方針：
 * - 既存フィールドの配置を壊さないため、verは構造体の末尾に追加
 * - v1.73a/b の古いEEPROMを読んでも、既存フィールドは正しく読み込める
 * - 新規フィールド（tmBusyActiveHigh, ver）は 0xFF になり得るため補完する
 */
struct MyConfig {
  uint32_t magic;
  ...
  uint8_t  tmBusyActiveHigh; // [g] 0=LOWが受信, 1=HIGHが受信（v1.73b〜）
  uint8_t  ver;           // ★ EEPROMレイアウト版（v1.73c〜）
} config;

const uint32_t CONFIG_MAGIC   = 0xDEADBEEF;
const uint8_t  CONFIG_VERSION = 3;   // v1.73c のレイアウト番号
...
const uint8_t ARD_RX_FROM_DFP = 10; // DFPlayer TX → Arduino RX
const uint8_t ARD_TX_TO_DFP   = 11; // DFPlayer RX ← Arduino TX
...
inline bool readBusy()  {
  ...
  return (tmBusyFiltered || a0Busy); // AUTO中は両方
}
...
void startPtt(uint16_t trk) {
  ...
  stateTimer = now;
  playingEnterAt = 0;  // PLAYING突入時に更新
  state = PTT_ON_WAIT;
}
...
void applyDefaults() {
  ...
  config.tmBusyActiveHigh = 1; // HIGH=busy
...
void loadSettings() {
  ...
  // 2) VERSION 判定（verが0xFF/未初期化/異なる場合は移行または補完）
  if (config.ver != CONFIG_VERSION) {
    Serial.print(F("[EEPROM] Version mismatch or uninitialized: stored="));
    Serial.print(config.ver);
    Serial.print(F(" expected="));
    Serial.println(CONFIG_VERSION);

    // --- 既存値を極力活かしつつ、新規フィールド/不正値を補完 ---
    // dfpTimeoutMs の補完（0や異常に大きい値は既定へ）
    if (config.dfpTimeoutMs == 0UL || config.dfpTimeoutMs > 600000UL) {
      config.dfpTimeoutMs = 20000UL;
    }
    // tmBusyActiveHigh の補完（0/1以外は既定=HIGH=busy）
...
void setup() {
  ...
  pinMode(PIN_TM_BUSY, INPUT_PULLUP); // 安定化：プルアップ運用
  ...
  Serial.println(F("[START] OpenCCVoice v1.73c Unified/Safe (+Versioned EEPROM, +DFP Timeout Cmd, +TM Busy Polarity Cmd)"));
  printSummary();
  printHelp();
}
