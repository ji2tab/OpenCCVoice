...
(5) EEPROM 永続化
  - m / n / b / i / p / r / L / G / a / w / s / t / d の変更で即保存。
...
(7) フェイルセーフ（PTT張り付き防止）
  - PLAYING 以降、DFPlayer の BUSY 応答が出ない/戻らない場合、
    設定された d####（既定 20000ms = 20秒）経過で PTT OFF へ遷移。
  - d0 で無効化可能（無効時は判定自体をスキップ）。
...
- 安全性：フェイルセーフ（既定 20s, d#### で可変）。d0 で無効化。
...
- AUTO_WINDOW = 30 min
- D6 デバウンス = 5ms
- DFP_TIMEOUT_MS = 20000ms（20秒, コマンド d#### で変更, d0=無効）
...
- q                    … 現在値の要約表示（DFP_TIMEOUT_MS含む）
- H                    … 汎用プリセット適用（s0 / t0 / b3900）
- x / r                … 停止（Safe Stop）/ 再開（停止中のみ有効）
- F                    … Factory Reset（初期値へ戻し、EEPROM保存）
...
- 短発連続：10s 窓で 2回以上 → バースト抑止（BURST_SUP_MS）
- 送信後抑止：t1 & r#### で TX 後の受信無視
- フェイルセーフ：PLAYING → (now - playingEnterAt ≥ d####) → 強制 OFF（d0 時は評価スキップ）
- PTT ステート：
...
- D6 デバウンス 5ms（可変なし）
- EEPROM 書込みは「値変更時のみ」
- SRAM 制約（ATmega328P 2KB 内に収まる）

9. 試験（例）
- m0（D6 固定）：
  - 600ms 受信 → ID送出（短発）
  - 4000ms 受信 → 長話抑止
- AUTO：m2 → w1 → 観測満了 → [AUTO-FIXED] ログ & D13高速点滅（~3s）
- 送信後抑止：t1 / r3000 → 自局送出直後 3s の受信無視
- フェイルセーフ（既定20秒）：DFPlayer BUSY異常時、20秒経過で強制OFF
  - d0 設定時は無効（PLAYING中の判定スキップ）

10. リスク
- DFPlayer BUSY の個体差（LOW/HIGH の応答タイミング差）
- 外部回路のプルアップ不足による D6/A0 の不安定
- A0/D6 の同時配線は禁止（片側のみ運用）  
  ※AUTO は観測/固定ロジックであり、配線は常に片側
