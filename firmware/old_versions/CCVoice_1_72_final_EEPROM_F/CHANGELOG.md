...
/************************************************************
 * OpenCCVoice Guidance Controller  (Unified & Configurable)
 * Version : 1.72-H1-Compatible-Safe
 * Target  : Arduino Nano (ATmega328P, 5V)
 *
 * 【本版について】
 * - v1.69g のハード配線（RX=10 / TX=11）を維持したまま動作
 * - 入力は INPUT_PULLUP（浮き対策）
 * - 既定値は H1 仕様（BUSY_MAX_MS=3900ms）
 * - DFPlayer 応答なし時に 8 秒で強制停止（フェイルセーフ）
 * - EEPROM 永続化 / Factory Reset（F）/ AUTO 判定固定 / STOP（x）/ RESUME（r）
 *
 * 【操作・設定ガイド（115200bps）】
 *  - モード       : m0=D6固定 / m1=A0固定 / m2=AUTO（w分の観測後に固定保存）
 *  - 受信しきい値 : n500（最短ms）/ b3900（最長ms＝LONG）
 *  - 抑止        : s0/s1（OFF/ON）
 ...
 *  - 要約/停止   : q / x→停止, r→再開
 *  - プリセット   : H（s0/t0/b3900）
 *  - 初期化       : F（Factory Reset：初期値に戻す＆EEPROM保存）
 *
 * 【注意】
 * - A0とD6は同時接続不可（片側のみ配線）。
 ************************************************************/
...
/* ============================================================
   ■ ピン定義（v1.69g配線に準拠）
   ============================================================ */
const bool TMBUSY_ACTIVE_HIGH = true;
...
const uint8_t PIN_TM_BUSY = 6;     // D6（BUSY）
const uint8_t PIN_DFP_BSY = 7;     // D7（DFP BUSY: 多くの個体で LOW=再生中）
const uint8_t PIN_DFP_OUT = 2;     // D2（BUSYミラー）
const uint8_t PIN_SUP_LED = 13;    // D13（抑止/AUTO通知）
const uint8_t PIN_A0_LED  = 12;    // D12（A0表示）

// ここが重要：v1.69g の配線を変えずに動かすため、
// SoftwareSerial の引数（(RX,TX)）に対して「PIN_DFP_TX=10 を RX として使用」します。
const uint8_t PIN_DFP_TX  = 10;    // D10（実際は SoftwareSerial の RX として動作）
const uint8_t PIN_DFP_RX  = 11;    // D11（実際は SoftwareSerial の TX として動作）
const uint8_t A0_PIN      = A0;

#define USE_A0_FRONT 1

// v1.69g互換（配線そのまま）で動作させるための初期化
SoftwareSerial dfpSerial(PIN_DFP_TX, PIN_DFP_RX);  // (RX=10, TX=11)
...
// DFPlayer応答なし時のフェイルセーフ（最大再生時間）
const unsigned long DFP_TIMEOUT_MS = 8000;
...
void handleSerialCmd() {
  while (Serial.available()) {
    ...
    switch (c) {
      ...
      case 'H': SUPPRESSORS_ENABLED=false; TX_AFTER_SUPPRESS_ENABLED=false; BUSY_MAX_MS=3900; LONG_TALK_MS=3900; break;
      case 'x': {
      ...
      case 'F': {
        applyDefaults();
        saveSettings();
        Serial.println(F("[RESET] Factory defaults restored (H1 Spec)."));
        chg=false; break;
      }
...
void setup() {
  ...
  Serial.println(F("[START] OpenCCVoice v1.72-H1-Compatible-Safe (HW:v1.69g / Logic:v1.72 / +Failsafe)"));
  printSummary();
}
  ...
  switch (state) {
    case PTT_ON_WAIT:
      if (now-stateTimer >= PTT_PRE_MS) {
        ...
        state = PLAYING;
        playingEnterAt = now;  // フェイルセーフ開始時刻
      }
      break;
