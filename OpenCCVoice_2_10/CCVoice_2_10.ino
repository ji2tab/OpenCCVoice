 ...
 * Version : 2.10  (RTC-Aligned Periodic ID)
 * Target  : Arduino Nano (ATmega328P, 5V)
 *
 * 【v2.10 変更点】
 * - DS3231 RTC モジュール対応（Wire.h / I2C A4=SDA, A5=SCL）
 * - 周期ID を「毎正時アライン」方式に変更
 *   p10/p15/p30 等、60の約数のとき → RTC実時刻ベースで正時アライン
 *   60の約数でない場合 / DS3231未接続時 → millis()相対動作（v2.01互換）
 * - T コマンド追加: 時刻設定 T20260311143000（YYYYMMDDHHmmss）
 * - u0/u1 コマンド追加: RTCアライン OFF/ON
 * - q（設定一覧）に現在RTC時刻を追加表示
 * - EEPROM CONFIG_VERSION = 5（v2.01=4 からの自動マイグレーション）
 *   ※ rtcAlignOn フィールドを追加（既定: 1=ON）
 *
...
const uint8_t ARD_RX_FROM_DFP   = 13;
const uint8_t ARD_TX_TO_DFP     = 12;
...
void setup() {
  ...
  Serial.println(F("[START] OpenCCVoice v2.10 (RTC-Aligned Periodic, Ver.5 pinmap)"));
  printSummary();
  printHelp();
}
