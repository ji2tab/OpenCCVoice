...
void handleSerialCmd() {
  ...
  while (Serial.available()) {
    ...
    switch (c) {
      ...
      // Vコマンド：スケッチバージョンとEEPROM_VERを表示（v1.73f追加、v1.73f1でV大文字に変更）
      case 'V':
        Serial.print(F("[VER] OpenCCVoice v1.73f1 (EEPROM_VER="));
...
void setup() {
  ...
  Serial.println(F("[START] OpenCCVoice v1.73f1 Unified/Safe (Refactored)"));
  printSummary();
  printHelp();
}
