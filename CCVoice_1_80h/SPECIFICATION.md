...
- n####                … ① 最小受信長（ms）（100〜）
- b####                … ② 最大受信長（ms）（500〜）
- j####                … ③ PTT先行無音時間 PRE（ms）（100〜）★v1.80h追加
- J####                … ④ PTT後行無音時間 POST（ms）（100〜）★v1.80h追加
- r####                … ⑤ 抑止時間（ms）（0〜）
- p##                  … ⑥ 定周期ID（分 / 0=停止）
...
- 出力例：[VER] OpenCCVoice v1.80h (EEPROM_VER=5)
...
- バージョン確認
  V → "[VER] OpenCCVoice v1.80h (EEPROM_VER=5)" を確認
