...
## v1.80i — コード整理・マイグレーション修正・デッドコード削除 (2026-03-24)

EEPROM CONFIG_VERSION は 5 のまま変更なし。v1.80h からの書き込みでそのまま動作する。

### Fixed（バグ修正・コード整理）

- **カーチャンク検出時の `busySupUntil` 二重セットを修正**
  - v1.80h の `processBusyLogic()` では、カーチャンク検出時と `processStateMachine()`
    の POST終了（起点A）の二か所で `busySupUntil` をセットしていた。
  - 動作結果は正しかったが設計意図が不明瞭だったため、カーチャンク検出時の先行セットを削除し、
    抑止セットを POST終了（起点A）に一本化した。

- **`migrateOrInit()` の ver=4 以外からのマイグレーション時の不定値リスクを修正**
  - v1.73c 以前（ver=3 以下）のデバイスから直接 v1.80i を書き込んだ場合、
    `pttPreMs` / `pttPostMs` が EEPROM 上に存在しないため読み出し値が不定だった。
  - `ver == 4` 以外の `else` ブランチでも `pttPreMs` / `pttPostMs` を 1000ms で
    明示初期化するよう修正（その後の範囲チェックによる二重安全も維持）。

- **`handleStoppedState()` 内のデッドコードを削除**
  - STOP中の `R` コマンド受付は `handleSerialCmd()` の `case 'R':` で完結しており、
    `handleStoppedState()` 内の `Serial.read()` ブロックは実質到達しないコードだった。
  - 該当コードを削除し、コメントで復帰処理の所在を明示した。

### Changed（コメント修正）

- 概要コメント「抑止（長話/連打/送信後）を統合管理」→ 正確な表現に修正
- `processBusyLogic()` 内「②超え → 長話抑止（起点B）」→ 正確な表現に修正
- `pAct=true` 時の ② 超え受信終了も起点B として `busySupUntil` をセットする旨をコメントに明記
- `handleSerialCmd()` / `setup()` の内部開発版番号（v1.80d/e）を v1.80h に修正

### EEPROM

- `CONFIG_VERSION` = 5（変更なし）
- 構造体フィールドの変更なし
...
## 付録：ピンマップ（Arduino Nano / Ver.5 PCB / v1.80i 時点）
...
| バージョン | 主な追加・改善 | 状態 |
|-----------|---------------|------|
| v1.73c | EEPROM 自動移行・安全補完 | ⚠️ 廃止 |
| v1.73d | 定周期ID 静寂ガード（k####） | ⚠️ 廃止 |
| v1.73e | 送信直後ガード・周期ID catch-up 対策 | ⚠️ 廃止 |
| v1.73f | `V`（バージョン確認）・`Z`（ソフトウェアリセット）コマンド追加 | ⚠️ 廃止 |
| v1.73f1 | `v` → `V` コマンド変更（v2.x 系との競合回避） | ⚠️ 廃止 |
| v1.80h | 仕様刷新（抑止一本化・PRE/POST可変化・スキップ仕様・バグ修正） | ⚠️ 廃止 |
| **v1.80i** | **コード整理・マイグレーション修正・デッドコード削除** | **✅ 現行** |
