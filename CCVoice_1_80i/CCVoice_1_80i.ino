 ...
 * - 抑止は ⑤busySupUntil で一元管理
 *   （カーチャンク後・通常会話終了・POST終了をすべて統合）
 * - PTTガード、BUSYデバウンスを実装
 ...
 * 【バージョン履歴】
 * ============================================================
 * v1.80i 変更点（v1.80h → v1.80i）：
 *
 *   [バグ修正・コード整理]
 *   - カーチャンク検出時の busySupUntil 先行セットを削除
 *     旧実装では processBusyLogic() でのカーチャンク検出時と
 *     processStateMachine() の POST終了（起点A）の二か所で
 *     busySupUntil をセットしていた（二重セット）。
 *     抑止セットを起点A（POST終了時）に一本化し、設計意図を明確化。
 *   - migrateOrInit() の ver=4 以外からのマイグレーション時にも
 *     pttPreMs / pttPostMs を明示的に 1000ms で初期化するよう修正
 *     （ver=3 以前からの直接書き込み時の不定値リスクを排除）
 *   - handleStoppedState() 内のデッドコードを削除
 *     STOP中の R コマンド受付は handleSerialCmd() の case 'R': で
 *     完結しており、handleStoppedState() 内の Serial.read() は
 *     実質到達しないデッドコードだったため削除。
 *
 *   [コメント修正]
 *   - 概要コメント「抑止（長話/連打/送信後）を統合管理」を正確な表現に修正
 *   - processBusyLogic() 内「長話抑止（起点B）」を正確な表現に修正
 *   - pAct=true 時の② 超え受信も起点B になる旨をコメントに明記
 *   - handleSerialCmd() / setup() の内部開発版番号（v1.80d/e）を v1.80h に修正
 *
 *   [EEPROM]
 *   - CONFIG_VERSION は 5 のまま変更なし
 *     （EEPROM 構造体フィールドの変更なし）
 *
 ...
 * 【リリース履歴】
 * ============================================================
 *  v1.80h  仕様刷新・抑止ロジック統合・PRE/POSTバグ修正
...
void migrateOrInit() {
  ...
  if (config.ver != CONFIG_VERSION) {
    ...
    if (config.ver == 4) {
      // v1.73系（ver=4）からのマイグレーション:
      // pttPreMs/pttPostMs は ver=4 の構造体に存在しないため必ず初期化する。
      // busyMax/txSupMs は旧デフォルト値の場合のみ新デフォルトへ更新する。
      ...
      // ver=3 以前など未知のバージョンからのマイグレーション:
      // pttPreMs/pttPostMs が EEPROM 上に存在しない可能性があるため
      // 不定値（ゴミ）を安全値で上書きする。
      config.pttPreMs  = 1000;
      config.pttPostMs = 1000;
    }
...
void printHelp() {
  Serial.println(F("---- OpenCCVoice v1.80i HELP ----"));
...
void handleSerialCmd() {
  ...
  while (Serial.available()) {
    ...
    switch (c) {
      ...
      case 'V':
        Serial.print(F("[VER] OpenCCVoice v1.80i (EEPROM_VER="));
 ...
 *
 * 【抑止セットの方針】
 *   - ② 超え（通常会話）終了時: 起点B として busySupUntil をここでセットする。
 *     pAct=true（PTT送出中）の場合でも ② 超え受信の終了は起点B として扱う。
 *   - カーチャンク検出時: busySupUntil は ここではセットしない。
 *     抑止セットは processStateMachine() の POST終了（起点A）に一本化する。
 *     これにより「PTT送出完了後から⑤抑止時間」という仕様を正確に実現する。
 */
  ...
  if (rB) {
    ...
    if (tmBusyStart != 0) {
      ...
      if (dur >= BUSY_MAX_MS) {
        ...
        // POST終了時に受信中だった場合も同様にここで処理済みとなる。
...
void handleStoppedState() {
  ...
  // R コマンドによる復帰は handleSerialCmd() の case 'R': で処理される。
  delay(5);
}
...
void setup() {
  Serial.begin(115200);
  Serial.setTimeout(50);   // v1.80h: parseInt()ブロッキング上限を50msに固定
  ...
  Serial.println(F("[START] OpenCCVoice v1.80i"));
