 ...
 * Version : 2.24f (Add V/Z commands from v1.73f1)
 * Target  : Arduino Nano (ATmega328P, 5V)
 *
 * 【v2.24f 変更点】
 * - `V` コマンド追加：バージョン確認（スケッチver + EEPROM_VER 表示）
 * - `Z` コマンド追加：WDTによるソフトウェアリセット（EEPROM変更なし）
 *   v1.73f1 由来。`F` → `Z` で完全クリーン起動が可能。
 *
 * 【v2.24 変更点】
 * - volatile キーワードの最適化（メモリアクセスの効率化）
 *   非同期割り込みを使用していない変数から volatile を削除
 * - ヘッダファイル（ccvoice_log.h / ccvoice_config.h）の関数に
 *   inline を付与し、将来的な多重定義エラーを防止
 *
 * 【v2.23 変更点】
 * - v2.22 のコンパイルエラーを修正
 *   enum BusySrc / enum LogLvl の定義順を struct より前に移動
 * - DFPlayer TX/RX ピンアサイン修正（v2.22 より継続）
 *   ARD_RX=D12 / ARD_TX=D13（配線図 Ver.5 に合わせる）
 *
 * 【v2.21 変更点】
...
#include "ccvoice_log.h"

/* =========================== Runtime Params ======================= */
BusySrc BUSY_INPUT_SOURCE;
...
bool isSuppressedNow(unsigned long now) {
...
void printHelp() {
  ...
  Serial.println(F("V=version  Z=SW reset(WDT,EEPROM変更なし)"));
...
void handleSerialCmd() {
  while (Serial.available()) {
    ...
    // Vコマンド：バージョン確認（v1.73f1由来）
    if (c == 'V') {
      Serial.print(F("[VER] OpenCCVoice v2.24f (EEPROM_VER="));
      Serial.print(config.ver);
      Serial.println(F(")"));
      continue;
    }

    // Zコマンド：ソフトウェアリセット（WDT, v1.73f1由来）
...
void setup() {
  ...
  Serial.println(F("[START] OpenCCVoice v2.24f (Event Log, Dual EEPROM, RTC-Aligned Periodic, Ver.5 pinmap)"));
  printSummary();
  printHelp();
